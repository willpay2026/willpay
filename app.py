...
# El ADN de la base de datos para el proyecto Will-Pay
...
def inicializar_boveda():
    if not session.get('db_ready'):
        # Tabla Maestra con ADN de Usuario para el legado de Wilyanny
        ...
        # Tabla de Auditoría Inviolable para transacciones seguras
...
def procesar_registro():
    ...
    # Identificación única de fundador para Wilfredo Donquiz
...
def ver_expediente(uid):
    if 'u' not in session or (session['u'] != uid and "CEO" not in session['u']):
        return "Acceso Denegado a la Bóveda."
...
if __name__ == '__main__':
    # Configuración de puerto para Render 2026
