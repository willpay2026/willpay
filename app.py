...
def procesar_registro():
    ...
    try:
        ...
        return f"<h1>✅ ¡Bienvenido {nombre}!</h1><p>Tu ID DNA es: {id_dna}</p><a href='/acceso'>Entrar</a>"
...
def dashboard():
    ...
    if u['cedula'] == '13496133': return render_template('ceo_panel.html', u=u, m=m)
...
def instalar():
    ...
    query_db("""CREATE TABLE movimientos (
        id SERIAL PRIMARY KEY, usuario_id INTEGER, correlativo TEXT, monto_bs FLOAT, fecha TIMESTAMP DEFAULT CURRENT_TIMESTAMP)""", commit=True)
    query_db("""INSERT INTO usuarios (id_dna, nombre, telefono, cedula, pin, actividad_economica, saldo_bs) 
        VALUES ('CEO-001', 'WILFREDO DONQUIZ', '04126602555', '13496133', '123456', 'FUNDADOR', 100.0)""", commit=True)
    return "<h1>🏛️ Bóveda Reseteda y Blindada</h1><p>Entra con 13496133 y PIN 123456</p>"
